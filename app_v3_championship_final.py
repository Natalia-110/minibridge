...
if (n+e+s+w) == 40:
    ...
    if not lock:
        ...
        if st.button("✅ 鎖定合約，開始打牌", use_container_width=True):
            ...
            if conflict:
                st.error(f"🚨 嚴重衝突警告：您輸入的四家點力與【{OTHER_ROOM} 室】紀錄不一致！")
            else:
        ...
        if st.button("💾 結算並儲存成績", use_container_width=True):
            if tricks > 13:
                st.error("🚨 嚴重錯誤：一副牌最多只有 13 磴！系統已攔截存檔，請修正。")
            else:
...
if st.session_state.history:
    ...
    st.download_button(label=f"📥 下載 {ROOM} 室 CSV", data=csv, file_name=f"bridge_{ROOM.lower()}.csv", mime="text/csv")