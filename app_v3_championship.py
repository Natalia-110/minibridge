# ---------------------------------------------------------
# 專案名稱：迷你橋牌大會計分系統 - V3 決賽旗艦版
# 學習歷程亮點：
# 1. 空間隔離：Open/Closed Room 實體檔案分流，確保資訊安全。
# 2. 自動判斷：根據 HCP 點力自動決定莊家。
# 3. 跨系統防呆：鎖定合約前，自動讀取另一室檔案比對點力，不符則阻擋。
# ---------------------------------------------------------
...
board_no = col_b.number_input("🔢 牌號 (Board No.)", 1, 32, 1, disabled=lock)
    ...
    if not lock:
        ...
        if st.button("💾 結算並儲存成績", use_container_width=True):
            ...
            st.session_state.history.append({
                ...
                "N_HCP": n, "E_HCP": e, "S_HCP": s, "W_HCP": w # 隱藏點力供日後檢核
            })
